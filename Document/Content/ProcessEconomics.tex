...
\refsec{sec:InvestmentCriteria}. In addition to the cost of raw materials needed during process
...
\eq{
	...
	\nomenclature{$C_{FILL}$}{Cost of raw materials to fill the process \nomunit{\$}}
}
...
and consider the project as a singe time periods (\refsec{sec:SinglePeriod}) and models that take into 
account cash flows made at different points in time -- so called multi-period models 
(\refsec{sec:MultiPeriod}).  
...
given project is shown in \reffig{fig:CashFlow}. When the project commences no revenue is generated 
...
\stdfig{pgfplots/ProjectCashFlow}{Accumulated cash flows over project life cycle.}{fig:CashFlow}{}
\todo[inline]{add ref: (\reffig{fig:CashFlow}) Script PE-VT}
...
\eq{
	C_n = C_0 \cdot q^n. 
	\nc{$q$}{Interest factor}{-}
	\nc{$C_n$}{Final value of an investment}{\$}
	\nc{$C_0$}{Initial value of an investment}{\$}
}%
...
\eq{
	...
	\nc{$C_{0e}$}{Present value of all expenses}{\$}
}%
...
\eq{
	...
	\nc{$C_{0r}$}{Present value of all revenues}{\$}
}%
