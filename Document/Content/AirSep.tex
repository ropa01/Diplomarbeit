...
There are several ways besides cryogenic air separation that can be employed to separate gaseous mixtures.
In general these rely on three basic principles by which separation can be achieved, namely 
...
\Figref{fig:tech_compar} illustrates the most economically viable processes depending on product
...
\label{sec:cryo_air_sep}
    ...
    As can be seen in \figref{fig:ASU} double effect heat integrated distillation column lies at the heart of the air
...
%    \end{figure}
    ...
    two bed cycle is shown in \figref{fig:PSA}. Ambient air is first led through the first reaction vessel at the
...
\section{Gas permeation}
    ...
    quantity highly pure product streams. As \figref{fig:tech_compar} illustrates the main application
...
%    \stdfig{pgfplots/GasPermeation}{Membrane unit for gas permeation.}{fig:gas_permeation}{}
    ...
    \figref{fig:gas_permeation} shows the schematic for a single stage membrane unit. Within the feed stream
