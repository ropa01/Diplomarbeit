    ...
    \label{sec:mathpro:steady:distmodel}
        ...
        \todo{correct stage super-structure, remove liquid and vapour feed}
        ...
        \Eqml{eq:col:CompBalance_opt}{
    		0 = \left(1 + s_j^V\right) \cdot V_j \cdot y_{i,j} + \left(1 + s_j^L\right)
                ...
                - \sum_{k=1}^{n_F} \zeta^F_{kj} \cdot F_j \cdot z_{i,j} - \zeta^R_j \cdot V_{n_S} \cdot y_{in_S}, \hfill%
    	...
    	\Eqml{eq:col:EnergyBalance}{
    		0 = \left(1 + s_j^V\right) \cdot V_j \cdot h^V_{j} + \left(1 + s_j^L\right)
                ...
                - \sum_{k=1}^{n_F} \zeta^F_{kj} \cdot F_k \cdot h^{F}_{j} - \zeta^R_j \cdot V_{n_S} \cdot h^V_{n_S} - Q_j, \hfill
        ...
        tied to dynamics, they will be discussed in closed detail in that context (\secref{chp:dynamics}).
